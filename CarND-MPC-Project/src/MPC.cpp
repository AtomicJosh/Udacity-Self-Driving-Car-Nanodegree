...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
     ...
     */
    fg[0] = 0; // Cost value, the below additions contribute to this cost function

    // The part of the cost based on the reference state.
    for (uint t = 0; t < N; t++) {
      fg[0] += CppAD::pow(vars[cte_start + t], 2) * 3000;
      fg[0] += CppAD::pow(vars[epsi_start + t], 2) * 3000;
    ...
    for (uint t = 0; t < N - 1; t++) {
      ...
      fg[0] += CppAD::pow(vars[delta_start + t] * vars[v_start+t], 2) * 700;
    }
    ...
    for (uint t = 1; t < N; t++) {
      ...
      // Use actuations from two timesteps prior to account for latency.
      if (t > 1) {
      ...
      AD<double> psides0 = CppAD::atan(coeffs[1] + 2 * 
                                       coeffs[2] * x0 + 3 * 
